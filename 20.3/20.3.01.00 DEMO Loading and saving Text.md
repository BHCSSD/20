# 20.3.01.00 DEMO Loading and saving Text
```
20.3.01.00 DEMO Loading and saving Text
```

**Things to focus on:**
* Making 2 different types of HTML buttons, generic, and upload buttons
...
function draw() {
  background(240);
  // 2. Only display text if the variable is full
if(inputBox){
...
3. **Add a Button for Saving the File**
   ...
   * Tell the button what to do when clicked using `saveButton.mousePressed(saveText)`, this means it will run a function called `saveText()`.
    * the `saveText` is a function name. Notice you do not use () with this one. 

4. **Define the `saveText()` Function**
