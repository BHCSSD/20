...
You can use the `movePlayer` and `checkcollisions` from the DEMO

### **Step 6: Higher Level 80%+**

#### Pacman-ish
1. Make `food.json` to store the x,y for food particles
2. use a function with a for loop to draw all your food particles
3. Make a `foodCollisions` function and use `get()`
    * When you run over the food, it gets eaten, your points go up and the food disapears.
4. Look at the p5 refference for `arc` and use their "pacman" example to animate your sprite.
5. Change your movement function to allow your pacman to rotate and move forward... see unit 1 four corners assignment 
