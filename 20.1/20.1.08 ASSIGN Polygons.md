...
> **Outcomes:**
> 
> * **2.4 Identify and apply control flow mechanisms:** Use loops and conditional statements within modular functions.
> * **3.1 Implement linked subprograms:** A coordinating function calls helper functions to produce the final output.
> * **3.3.1 Use parameters to generalize subprograms:** Multiple parameters allow one function to handle many polygon variations.
...
### TODO: Setup
1. Make an object that stores **ALL** the values for your polygon:  
...
2. Use [createSlider](https://p5js.org/reference/p5/createSlider/) to alter the speed of the rotation.  
   ...
   - your polygons should be able to fowards and backwards. 

---

### TODO: Part 2
1. **Size Control with [keyCode](https://p5js.org/reference/p5/keyCode/) and `scale`**  
   - If you press the `UP_ARROW`, the size increases by 0.25 up to 2.5x, then resets back to 0.25x Size.  
   - If you press the `DOWN_ARROW`, the size decreases by 0.25 , and after it hits 0.25, it resets back to 1x.  
   - Display the current size in a large font at the top.
     - Make sure to use the [string templates](https://www.w3schools.com/js/js_string_templates.asp) 
   - Use p5js refference for `scale()` to help.
     - This will be inside the `push` `pop` you have for your translate.    
