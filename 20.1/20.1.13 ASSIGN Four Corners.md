...
>* **numeric keycodes:** Only use these if you are using version 1.x   [keycode chart](https://www.toptal.com/developers/keycode) to find any key’s number. Else use `"w"`
> * **MEGA HINT**  look a the p5 refference for [keyIsDown()](https://p5js.org/reference/p5/keyIsDown/)

### **1.1. Sprite Movement but better**
  Partial hint on how to rotate your sprite to face the angle of motion: 
  ```js
  if (keyIsDown(87)) { // 'W' key
    sprite.x += cos(sprite.angle) * sprite.speed;
    sprite.y += sin(sprite.angle) * sprite.speed;
  }
```
