...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Q1: Balloon**              | * Draw at least one balloon using a function `drawBalloon(size,col)`.<br>* Balloon size should respond to `size`. Use one way to make this ineractive, mourseDragged, key pressed ect. Use `text()` to indicate how you chose to make it interactive. If you use your mouse, make sure it only work inside of Q1. <br>* Include the knot of the balloon and a string<br>* Use 4 lines to show a "reflection cross hatch" on your balloon. |
...
| **Q3: Splash**            | * Draw a blue water background using `background()`.<br>* When clicking in Q3 only, draw a splash using `drawSplash(cx, cy)`.<br> * Use at least 4 semi-transparent ellipses, `fill(v1, v2, v3, [alpha])`.<br>* Vary size and alpha for layered effect. <br>* use `random()` to make the circle grow and shrink a little bit   <br>* think of a ripple in a calm pond after you throw a peble 
| **Q4: Umbrella**          | * Draw an umbrella using `drawUmbrella(col)`.<br>* To draw the canopy use `arc()`,  include the handle and use a few `lines()` to show rain `strokeWeight()`.    <br>* reminder `noFill` and `stroke` will be usefull here                    |
