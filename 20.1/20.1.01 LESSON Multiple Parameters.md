...
* A. The code runs fine and draws a 99 but no circle.
* B. Nothing happens.
* C. An error is shown in the Console.
* D. The code asks the user to provide the missing colour argument and uses that colour.
...
* A. `'yellow' is not a number.`
* B. You did something wrong. Please try again.
* C. `circle.fill` should be colour.
* D. No error.
...
* A. You did something wrong. Please try again.
* B. No error is shown, but nothing happens.
* C. No error occurs, and two circles are drawn.
* D. `drawCircles` call requires parentheses.
