...
But `getRandomcolour()` doesn’t exist yet…  we need to make it

##  Writing `getRandomcolour()`

1. Step 1: Make a list of colours
    * We’ll create a **local array** that holds possible colour options.
      * You do not know arrays yet. it is a set of `[]` that hold info
        ```js
        let arr = ["col1","col2","col3"]// as an example.
        ``` 
...
function getRandomcolour() {
  let colours = ['red', 'orange', 'yellow', 'green', 'blue']// hey look, a local list
  let colourChoice = random(colours); // hey look, a local variable
...
>   * some functions have different return values inside of `if` statements
...
> ```
> this way was ok last year, but you can only have 1 `sum` variable. With the funciton you can reuse it billions of times with out worrying about variable names. 
> 
