...
> |------|------|------|
> | `.push()` | Add item to the **end**  | `enemies.push("dragon")` |
> | `.unshift()` | Add item to the **start** | `enemies.unshift("dragon")` |
> | `.splice()` | Add/remove items at a **specific index** | `enemies.splice(1, 0, "dragon")` |
> | `.pop()` | Remove **last item** | `enemies.pop()` |
> | `.shift()` | Remove **first item** | `enemies.shift()` |
> | `.includes()` | Check if the list **contains an item** | `enemies.includes("dragon")` |
> | `.indexOf()` | Find the **index of an item** | `enemies.indexOf("goblin")` |
> | `.length` | Get the **number of items** in the list | `enemies.length` |
>
...
> | --------------------------- | ------------------------------ | -------------------------------------------------------------------- |
> | `arr.includes(value)`       | Value **exists** in the array  | `if (arr.includes("dog")) { console.log("Dog found"); }`             |
> | `!arr.includes(value)`      | Value **does NOT exist**       | `if (!arr.includes("fish")) { console.log("Fish not found"); }`      |
> | `arr.indexOf(value) !== -1` | Value **exists** (using index) | `if (arr.indexOf("dog") !== -1) { console.log("Dog found"); }`       |
> | `arr.indexOf(value) === -1` | Value **does NOT exist**       | `if (arr.indexOf("fish") === -1) { console.log("Fish not found"); }` |
...
- In `mousePressed`, simply increment `sceneNum` 
...
### Scene 9
Your choice. Use an `if` statement in a meaningful way. Make sure you say in a `text` what your user needs to do to make a choice.
