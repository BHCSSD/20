...
> * **1.1.2 Advantages and disadvantages of fundamental data structures:** Using one array container is much better and more efficient than creating 50 separate variables for related items.
...
names to identify the location of data elements
> * **1.2.1 Static arrays (use of an index):** Using index numbers (starting at 0) and brackets like `colourList[0]` to access exactly which item you wanted.
> * **1.3.1 Creating the structure:** The syntax to declare and fill a new array using `let colourList = [...]`.

> [!IMPORTANT]
> ## After the first 2 lessons you will be doing both `20.2.00 PRACTICE Code Tracing 1` and `20.2.00 TEST Code Tracing 1` in D2L
> 
...
> Arrays are stored in **contiguous memory** blocks. This means when an array is created, the computer allocates a single, unbroken section of memory where every item sits directly next to the next one.
> * **The Base Address:** The computer only needs to track the memory address of the very first item (index 0).
> * **Offsets:** Because items are stored side-by-side and are of a fixed size, the computer doesn't need to "search" the list. It calculates the exact location of any item using a simple formula:
> `Address = Base Address + (Index * Item Size)`
> * **Efficiency:** This "random access" capability is why arrays are extremely fast for retrieving data if you already know the index.
> 
>
...
function setup() {
 ...
 textSize(32);
 background(220);
  text('Press the mouse to \ndisplay the next colour', width/2, height/2,)
}
...
function mousePressed() {
 ...
 textSize(22)
 text(`This is not the best code, \nnotice I did not use draw() \n this is just and example`,width/2, height/2+111) 
...
We can also **loop through** every item in a list
...
function draw() {
 ...
 for (let i = 0; i < colours.length; i++) {
   ...
   centerX += 60 // look at this... it moves
 }//end of for
}
